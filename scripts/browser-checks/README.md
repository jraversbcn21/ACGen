...
| `generator-tools.mjs` | 57 asertos (2026-09-02, PR #60 useGenerator): la pasada "manual" de los 9 tools de generacion con el modelo simulado en la pagina (SSE troceado con retardo): generar; regenerar con Ctrl+Enter y comprobar que el resultado anterior desaparece ANTES del segundo token y que el stream se ve en vivo donde hay vista de stream (el C1 que cazo la review final); Limpiar a mitad de stream sin que nada resucite; modo confidencial con renombrado [EMAIL_1] -> [PERSONA] comprobando lo que sale por la red y que el resultado vuelve deanonimizado. Acepta un segundo argumento con la view para correr un solo tool |
