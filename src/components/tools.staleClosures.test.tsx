...
const TOOLS: {
  ...
  { name: 'AcceptanceCriteriaTool', Tool: AcceptanceCriteriaTool, output: 'Criterios generados' },
  { name: 'BugReportTool', Tool: BugReportTool, output: 'Informe generado' },
  { name: 'ConverterTool', Tool: ConverterTool, output: 'Texto convertido' },
  { name: 'RefinerTool', Tool: RefinerTool, output: 'Requisito refinado' },
  { name: 'UserStoryTool', Tool: UserStoryTool, output: 'Historia generada' },
  { name: 'TestCaseTool', Tool: TestCaseTool, output: DEMO_DATA.testcase.output },
  { name: 'EdgeCaseTool', Tool: EdgeCaseTool, output: JSON.stringify([{ categoria: 'Limites', escenario: 'x', resultadoEsperado: 'y' }]) },
  {
    name: 'TestDataTool',
    Tool: TestDataTool,
...
function fillEveryTextbox() {
  screen.getAllByRole('textbox').forEach((el) => fireEvent.change(el, { target: { value: 'algo' } }));
}

describe.each(TOOLS)('$name — closures que no se quedan viejas', ({ Tool, output, props, prepare }) => {
