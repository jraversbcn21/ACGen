...
| `src/hooks/useRegressions.test.ts` | 13 — versioned model: add/update/delete a regression per platform, add/update/delete a ticket, archive one regression (moves it out of the live list into `archived` as a discriminated `ArchivedRegressionEntry`), delete archived, persistence+hydration incl. legacy `board`/old-style archived snapshots passed through intact (hydrated and re-persisted, never rendered), corrupt JSON |
| `src/components/RegressionCard.test.tsx` | 8 — collapsed by default (`defaultExpanded` override), toggles the ticket table, header edit (version/url/fecha) save/cancel, "Nombre - URL" ticket cell renders as an accent link (name overlay at rest, full value on focus, ctrl+click / ↗ icon open it), add ticket row, delete row (confirm only when the row has content), readOnly hides Editar/Archivar/Eliminar and the delete-row column |
| `src/components/RegressionTracker.test.tsx` | 8 — 2 platform tabs (APPS/WEB) independent regression lists, create a regression (version required, url/fecha optional), archive-one flow (confirm → card leaves the list → "Archivadas (1)" → snapshot opens the `RegressionCard` read-only and expanded), mixed history (a legacy full-board snapshot still opens the old read-only `TrackerGrid`), delete an archived entry → empty state |
...
**Total: 429 tests across 45 files.**
...
- **i18n**: `I18nContext` + `useT()` hook. `es.json` and `en.json` (240 keys, exact parity, guarded by `src/i18n/keyParity.test.ts`). Language toggle in Header (ES|EN). Detects browser language on first visit. Missing keys fall back to Spanish. Parameter interpolation supported.
...
- `RegressionCard.tsx`: collapsible header (version, "Nombre - URL" accent link opening in a new tab, formatted date, filled-ticket badge, Editar/Archivar/Eliminar) that expands into a ticket table — columns Ticket/Fecha/Prioridad/Creador/Squad. "+ Añadir ticket" appends rows; the per-row × asks `confirm` only when the row has any content (an empty row deletes silently). The Ticket column reuses the same "Nombre - URL" link-cell rendering as `TrackerGrid` (name overlay at rest, full value on focus, Ctrl+click or the ↗ icon opens it) via the shared `src/utils/trackerLinks.ts#parseUrlCell`
...
| `src/i18n/es.json` | 240 Spanish UI strings |
| `src/i18n/en.json` | 240 English UI strings — key set kept in exact parity with `es.json`, guarded by `src/i18n/keyParity.test.ts` |
