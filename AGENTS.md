...
**Total: 414 tests across 42 files.**
...
- **i18n**: `I18nContext` + `useT()` hook. `es.json` and `en.json` (224 keys, exact parity, guarded by `src/i18n/keyParity.test.ts`). Language toggle in Header (ES|EN). Detects browser language on first visit. Missing keys fall back to Spanish. Parameter interpolation supported.
...
| `src/i18n/es.json` | 224 Spanish UI strings |
| `src/i18n/en.json` | 224 English UI strings — key set kept in exact parity with `es.json`, guarded by `src/i18n/keyParity.test.ts` |
