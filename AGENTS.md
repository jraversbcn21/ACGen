...
**Total: 403 tests across 40 files.**
...
- **i18n**: `I18nContext` + `useT()` hook. `es.json` and `en.json` (219 keys, exact parity, guarded by `src/i18n/keyParity.test.ts`). Language toggle in Header (ES|EN). Detects browser language on first visit. Missing keys fall back to Spanish. Parameter interpolation supported.
