...
| `src/hooks/useRegressions.test.ts` | 17 — versioned model: add/update/delete a regression per platform, add/update/delete a ticket (incl. the `status` field), archive one regression (moves it out of the live list into `archived` as a discriminated `ArchivedRegressionEntry`), delete archived, persistence+hydration incl. legacy `board`/old-style archived snapshots passed through intact (hydrated and re-persisted, never rendered), malformed `archived` entries dropped without wiping state, `status` backfilled on tickets stored before the column existed, corrupt JSON |
| `src/components/RegressionCard.test.tsx` | 11 — collapsed by default (`defaultExpanded` override), toggles the ticket table, header edit (version/url/fecha) save/cancel, "Nombre - URL" ticket cell renders as an accent link (name overlay at rest, full value on focus, ctrl+click / ↗ icon open it), add ticket row, delete row (confirm only when the row has content), Status column ordered after Squad, column resize via header drag persisted to `acgen_regression_ticket_col_widths` (ephemeral in readOnly), readOnly hides Editar/Archivar/Eliminar and the delete-row column |
| `src/components/RegressionTracker.test.tsx` | 9 — 2 platform tabs (APPS/WEB) independent regression lists, create a regression (version required, url/fecha optional), archive-one flow (confirm → card leaves the list → "Archivadas (1)" → snapshot opens the `RegressionCard` read-only and expanded), mixed history (a legacy full-board snapshot still opens the old read-only `TrackerGrid`), delete an archived entry → empty state |
...
- **i18n**: `I18nContext` + `useT()` hook. `es.json` and `en.json` (241 keys, exact parity, guarded by `src/i18n/keyParity.test.ts`). Language toggle in Header (ES|EN). Detects browser language on first visit. Missing keys fall back to Spanish. Parameter interpolation supported.
...
| `src/i18n/es.json` | 241 Spanish UI strings |
| `src/i18n/en.json` | 241 English UI strings — key set kept in exact parity with `es.json`, guarded by `src/i18n/keyParity.test.ts` |
