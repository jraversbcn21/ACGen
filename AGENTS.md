...
- **Shared CSS primitives**: form fields (`.field-input`, `.field-select`, `.field-textarea`, `.field-label`), buttons (`.btn-primary`, `.btn-ghost`, `.btn-copy`, `.btn-icon-new`), tables (`.data-table-wrap`, `.data-table`), badges (`.badge` + `.badge-high/medium/low/positive/negative/info`), output panels (`.panel`, `.panel-header`, `.panel-body`), reasoning sections (`.reasoning` with `<details>/<summary>` + TTS controls: `.reasoning-tts`, `.tts-lang-btn`, `.tts-lang-btn.active`, `.tts-play-btn`, `.tts-stop-btn`), Jira config (`.jira-indicator`, `.jira-config`, `.jira-config-title`, `.jira-fields`), action bar (`.actions-bar`), model badge (`.model-badge-new`).
...
- **Main content** — asymmetric two-column grid (`.criteria-grid`, `grid-template-columns: 3fr 2fr`, gap 24px):
  - **Left column** (`.criteria-left`) — stacked: input textarea (`.field-textarea.criteria-input-ta`), output textarea (`.field-textarea.criteria-output-ta`, editable), "Copiar Criterio" button (`.btn-ghost.btn-copy`, green `--success` styling, right-aligned, visible only when output exists). Shows "¡Copiado!" feedback on click.
  - **Right column** (`.criteria-right`) — reasoning collapsible (`.reasoning` with `<details>/<summary>` + `.reasoning-body`). Only rendered when reasoning exists. Expand triggers `scrollIntoView` via `requestAnimationFrame`. The `<summary>` contains a flex row with the label (`.reasoning-label`) on the left and TTS controls (`.reasoning-tts`) on the right: ES/EN language toggle buttons (`.tts-lang-btn`) and play/stop buttons (`.tts-play-btn`/`.tts-stop-btn`) using `window.speechSynthesis`. Speech cancels on new generation, clear, or unmount.
...
- **Output area** (`.br-output-section`): read-only textarea (`.field-textarea.br-output-ta`, `min-height: 470px`), "Copiar bug report" button (`.btn-ghost.btn-copy`, green `--success` styling), reasoning collapsible (`.reasoning`) below with TTS controls (same layout as AcceptanceCriteriaTool — `.reasoning-tts` with ES/EN language toggle and play/stop buttons).
...
| `src/components/AcceptanceCriteriaTool.tsx` | Criteria UI: `.jira-config` section, `.criteria-grid` asymmetric columns (input→output→copy left, reasoning right with TTS controls), generate/clear/copy, history modal ("Historial" button with `.history-count` badge), dual input flow (Jira URL → fetch → format → generate, or plain text) |
...
| `src/components/BugReportTool.tsx` | Bug report UI: structured form (`.br-form-grid`, platform/market, dynamic web/app fields with `.field-select` device dropdowns), `.jira-indicator`/`.jira-config` for Jira, output textarea (`.br-output-ta`, min-height 470px) + copy + reasoning with TTS controls, history modal, generate/clear |
...
- Remaining per-tool layout classes in `App.css`: `.criteria-grid`, `.criteria-left`, `.criteria-right`, `.criteria-input-ta`, `.criteria-output-ta` (acceptance criteria asymmetric grid), `.br-form-grid`, `.br-form-row`, `.br-form-row-single`, `.br-form-field`, `.br-output-section`, `.br-output-ta` (bug report form layout), `.td-form-grid`, `.td-form-row`, `.td-form-row-single`, `.td-form-field`, `.td-output-section`, `.td-actions-bar`, `.td-copy-col`, `.td-copy-row-btn` (test data form/table layout), `.output-section`, `.output-header` (generic output area), `.copy-row` (copy button alignment, right-aligned), `.btn-copy` (green copy button, `--success` tokens, with hover fill), `.btn-copied` (copy success state, alternative green for TestCaseTool), `.loading-status` (inline loading text), `.steps-list` (ordered step list).
- Shared primitives in `App.css`: `.field-input`, `.field-select`, `.field-textarea`, `.field-label`, `.input-wrap`, `.select-chev`, `.adorn-btn`, `.btn-primary`, `.btn-ghost`, `.btn-copy`, `.btn-icon-new`, `.btn-loading`, `.spinner-new`, `.data-table-wrap`, `.data-table`, `.badge` + variants, `.panel`, `.panel-header`, `.panel-body`, `.reasoning`, `.reasoning-label`, `.reasoning-tts`, `.tts-lang-btn`, `.tts-play-btn`, `.tts-stop-btn`, `.jira-indicator`, `.jira-indicator-text`, `.jira-config`, `.jira-config-title`, `.jira-fields`, `.actions-bar`, `.model-badge-new`, `.error-banner`, `.error-icon`, `.error-text`, `.dismiss-btn`.
