...
== Plan: tracker-base-url (docs/superpowers/plans/2026-07-21-tracker-base-url.md), branch fix/tracker-base-url, base ff8a0ee ==
Task 1: complete (commits ff8a0ee..94bcda3, review clean, 378 tests)
  Minor (final review triage): duplicacion de logica de match del patron de ticket entre ticketKey y unconfiguredTicket (mutuamente excluyentes en runtime, cosmetico)
Task 2: complete (commits 94bcda3..c932e65, review clean, 381 tests)
  Minor (final review triage): (a) el efecto migra por condicion linkMode+vacio, no estrictamente on-mount (inerte hoy: ningun caller cambia de modo); (b) ramas defensivas de readLegacyBaseUrl (JSON malformado / no-string) sin test
Task 3: complete (commits c932e65..6ff2975, review approved tras 1 fix round — 3 Important: (1) el gear no cerraba el panel (mousedown->blur->save->close, luego click reabria) arreglado con onMouseDown que arma el flag; (2) borrador vacio escribia "" y la migracion de Task 2 resucitaba la URL legacy en el siguiente mount, ahora vacio = no-op; (3) el flag de cancelacion y el save-on-blur no estaban cubiertos, +4 tests)
  Minor (final review triage): (a) la rama Escape arma urlConfigCancelled pero es inalcanzable por diseno (React no dispara blur al desmontar) — el modelo mental del comentario no describe el codigo; (b) el test "Escape cierra sin guardar" no asserta nada real (blur sobre nodo desmontado); (c) una URL configurada ya no se puede limpiar desde la UI (decision deliberada, documentada en comentario); (d) setDraftBaseUrl(storedBaseUrl) pasa el valor crudo sin el guard "|| ''" de la linea 167 — una clave corrupta (p.ej. 123) haria throw en draftBaseUrl.trim(); (e) onMouseDown no filtra e.button===0 (click derecho consume el flag); (f) el input no tiene label accesible, solo placeholder
Task 4 (docs): complete (commit 6ff2975..b3eed1f, review clean, AGENTS.md/README a 389 tests/40 files, fila de historial verificada contra el codigo real)
FINAL: tracker-base-url complete (8 commits, c794201..dffaade, 390 tests/40 files, lint 0 err, build OK). Final review by opus: NEEDS WORK con 1 blocker — el propio input del gear aceptaba una URL sin esquema (jira.miempresa.com) y reproducia el bug original exacto; arreglado con doble capa (prefijo https:// al guardar + ABSOLUTE_HTTP_URL exigido en getLinkUrl, cubre tambien valores heredados por migracion) + 4 limpiezas. Re-review: READY TO MERGE. Ultimo detalle (color de aviso del gear con URL invalida) arreglado por el controlador.
  Deferred follow-ups: draft degenerado de solo barras normaliza a "https:" (no genera enlace, valor basura inocuo); input de config sin aria-label; ramas defensivas de readLegacyBaseUrl sin test; duplicacion cosmetica del match de patron de ticket
INCREMENT post-PR#18 (feedback Jorge en preview): hover ↗ icon abre enlaces con click directo + tooltips ensenan Ctrl+Click (commit dffaade..013e912, review approved solo minors, 396 tests/40 files)
  Minor (deferidos): (a) los ultimos ~22px de una celda larga los captura el boton en hover (caret vs abrir); (b) tactil sin :hover no ve el icono (ctrl+click ya era hostil en tactil, no-regresion); (c) icono presente tambien en snapshots readOnly (consistente, no spec-ado); (d) preventDefault de mousedown y tabIndex=-1 sin test que los pinee; (e) aria-label "Abrir el enlace" no unico entre celdas url
